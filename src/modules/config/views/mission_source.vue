<template>
  <div class="app-container" v-loading="loading">
    ...
    <div class="action-container">
      <el-button  icon="el-icon-plus" size="small" type="primary" @click="openCreatedDialog">添加任务来源</el-button>
...
<script>
  import {getTableList,deleteMethod,editMethod,createdMethod} from '@/modules/config/api/mission_source'
  ...
  export default {
    ...
    methods: {
      ...
      getTableList: function(number, pageSize) {
        ...
        getTableList(number, pageSize).then(response => {
          ...
          for (let i = 0; i < data.length; i++) {
            data[i].newIcons = returnThumbnailUrl(data[i].logoId)
          }
          this.tableData = data
          this.loading = false
        }).catch(() => {
      ...
      //编辑
      openEditDialog (row) {
        this.edit.missionSourceId = row.id
        this.edit.name = row.name
        this.edit.iconId = row.logoId
